    ...
    <>
      <Container>
        ...
        <StoreList>
          <StoreMenuList>
            {tabClick === 0 ? (
              <TabListActive onClick={() => { setTabClick(0) }}>티켓</TabListActive>
            ) : (
                <TabList onClick={() => { setTabClick(0) }}>티켓</TabList>
              )}
            {tabClick === 1 ? (
              <TabListActive onClick={() => { setTabClick(1) }}>스낵</TabListActive>
            ) : (
                <TabList onClick={() => { setTabClick(1) }}>스낵</TabList>
              )}
            {tabClick === 2 ? (
              <TabListActive onClick={() => { setTabClick(2) }}>음료</TabListActive>
            ) : (
                <TabList onClick={() => { setTabClick(2) }}>음료</TabList>
              )}
            {tabClick === 3 ? (
              <TabListActive onClick={() => { setTabClick(3) }}>콤보</TabListActive>
            ) : (
                <TabList onClick={() => { setTabClick(3) }}>콤보</TabList>
              )}
          </StoreMenuList>
        </StoreList>
        {/* Tab눌렀을 때 뜨는 메뉴 리스트 */}
...
const TabCont = (props) => {
  ...
  const tab = ['티켓', '스낵', '음료', '콤보'];

  return (
    !menu.ticket ?  // tickek의 데이터가 없으면(로딩중이면) 빙글빙글 , 다 받아오면 로직띄우기
      (<CircularProgress style={{
        position: "absolute", top: "25%", left: "50%", margin: "-150px 0 0 - 150px"
      }} />) : (
        tab.map((data, idx) => {
